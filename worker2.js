...
// --- KONFIGURASI CUSTOM PATH ---
// Ubah "/ritoko" menjadi path yang Anda inginkan.
// Contoh: "/vless", "/ws", "/ganteng"
let CUSTOM_PATH = "/ritoko"; 
// -------------------------------
...
export default {
  async fetch(request, env, ctx) {
    try {
      ...
      // Cek Environment Variable untuk Custom Path (opsional)
      if (env.CUSTOM_PATH) {
      ...
      if (upgradeHeader === "websocket") {
        ...
        // === CUSTOM PATH LOGIC ===
        // Jika path URL sama dengan CUSTOM_PATH yang ditentukan
        if (path === CUSTOM_PATH) {
          const proxies = await getPrxList(env.PRX_BANK_URL);
          
          if (proxies.length === 0) {
             return new Response("No proxies available.", { status: 503 });
          }

          // Pilih proxy secara acak dari semua list (Global rotation)
          const randomProxy = proxies[Math.floor(Math.random() * proxies.length)];
          prxIP = `${randomProxy.prxIP}:${randomProxy.prxPort}`;
          
          console.log(`Selected Proxy (Custom Path '${path}'): ${prxIP}`);
